...
export const Route = createFileRoute("/pebbles")({
  head: () => ({
    meta: [
      { title: "Decorative Pebbles & Stones — Vassio" },
      {
        name: "description",
        content:
          "Polished marble pebbles, natural river stones, and landscaping rocks to accentuate planters, garden beds, and water features.",
      },
    ],
  }),
    ...
    <Layout>
      <div className="mx-auto max-w-[1400px] px-6 py-12 md:py-16">
        ...
        <nav className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground font-semibold mb-8 flex items-center gap-1.5">
          ...
          <span>/</span>
          <span className="text-foreground">Pebbles</span>
        </nav>
        ...
        <div className="mb-12 border-b border-border/30 pb-8">
          ...
          <h1 className="serif text-4xl md:text-6xl text-foreground">Decorative Pebbles & Landscaping Stones</h1>
          <p className="mt-4 text-sm text-muted-foreground max-w-2xl leading-relaxed">
            Curated river pebbles, polished white marble stones, and organic gravel toppings designed to cover soil, accentuate planters, and create calming zen gardens.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-card border border-border/30 rounded-2xl p-4 mb-10 shadow-xs">
          <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <SlidersHorizontal className="h-3.5 w-3.5" /> {pebbleProducts.length} Decorative Stone Accents
          </span>
