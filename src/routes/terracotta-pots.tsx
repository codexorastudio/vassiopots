...
import { products, vases } from "@/data/products";
...
export const Route = createFileRoute("/terracotta-pots")({
  head: () => ({
    meta: [
      { title: "Terracotta Pots & Earthware Planters — Vassio" },
      {
        name: "description",
        content:
          "Handcrafted natural terracotta and stone ceramic pots designed to radiate warmth, craft, and organic earthy textures.",
      },
    ],
  }),
    ...
    <Layout>
      <div className="mx-auto max-w-[1400px] px-6 py-12 md:py-16">
        ...
        <nav className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground font-semibold mb-8 flex items-center gap-1.5">
          ...
          <span>/</span>
          <span className="text-foreground">Terracotta Pots</span>
        </nav>
        ...
        <div className="mb-12 border-b border-border/30 pb-8">
          <p className="text-xs uppercase tracking-[0.4em] text-primary font-bold mb-2">
            Earthy Craft & Natural Clay
          </p>
          <h1 className="serif text-4xl md:text-6xl text-foreground">Terracotta & Earthware Pots</h1>
          <p className="mt-4 text-sm text-muted-foreground max-w-2xl leading-relaxed">
            Authentic terracotta vessels and stone-finished clay pots. Breathable, porous, and beautifully aged to complement organic modern interiors and garden spaces.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-card border border-border/30 rounded-2xl p-4 mb-10 shadow-xs">
          <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <SlidersHorizontal className="h-3.5 w-3.5" /> {terracottaProducts.length} Handcrafted Earthware Planters
          </span>
